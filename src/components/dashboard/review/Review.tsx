...
import { useGetAllReviews, useUpdateReviewStatus } from "../../../hook/review/useReview";
...
const Review = () => {
    ...
    const handleUpdateStatus = async (id: number, status: ReviewStatus) => {
        if (window.confirm(`Bạn có chắc muốn ${status === "APPROVED" ? "duyệt" : "từ chối"} đánh giá này?`)) {
                    ...
                    <div className="flex gap-2">
                        ...
                        {Array.from({ length: data.data.totalPage }, (_, i) => (
                            <button
                                key={i}
                                onClick={() => setPage(i)}
                                className={`shadow w-[30px] h-[30px] text-black rounded ${page === i ? "bg-blue-400 text-white" : ""
                                    }`}
                            >
                                {i + 1}
                            </button>
                        ...
                        <button
                            ...
                            className={` rounded border w-[30px] h-[30px] ${page === data.data.totalPage - 1
                                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                                    : "bg-blue-400 text-white"
                                }`}
                        >
